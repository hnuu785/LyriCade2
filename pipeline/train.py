...
from pipeline.rhyme import compute_rhyme_loss, normalize_lyrics_for_rhyme, prepare_rhyme_supervision
...
def create_input_sequences(df):
    ...
    for _, row in df.iterrows():
        feature_string = " ".join(
            [f"<{column}: {row[column]:.2f}>" for column in NUMERICAL_COLUMNS]
            + [f"<{column}: {row[column]}>" for column in CATEGORICAL_COLUMNS]
        )
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train a Korean lyric generation model from a cleaned CSV.")
    parser.add_argument("--csv", required=True, help="Path to the training CSV.")
    parser.add_argument("--output-dir", default="outputs/run", help="Directory where checkpoints will be saved.")
